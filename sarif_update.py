...
def get_input_sarif_filename() -> str:
    """
    Retrieves the SARIF filename from the command-line arguments (sys.argv[1]).
    """
    if len(sys.argv) < 2:
        logger.error("Usage: python sarif_update.py <SARIF_FILENAME>")
        logger.error("Error: No SARIF filename provided.")
        sys.exit(1)

    sarif_input_filename = sys.argv[1]

    return sarif_input_filename
...
def main():
    input_sarif_filename = get_input_sarif_filename()

    original_sarif_data = read_sarif_file(input_sarif_filename)
    ...
    output_sarif_filename = f"{input_sarif_filename.replace('.sarif', '')}-updated.sarif"
    write_sarif_file(output_sarif_filename, updated_sarif_data)
